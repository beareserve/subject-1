...
public class Demo6 {
	...
	public void suspendResumeTestV1() throws Exception {
		Thread consumerThread = new Thread(() -> {
			if (baozidian ==null) {
				System.out.println("1.进入等待");
				Thread.currentThread().suspend();
	...
	public void parkUnParkDeadLockTest() throws Exception{
		Thread parkThread = new Thread(() -> {
			if (baozidian == null) {
				System.out.println("1.没有包子，等它出锅");
				try {
					Thread.sleep(5000L);
				} catch (InterruptedException e) {
					e.printStackTrace();
			...
			System.out.println("3.被通知有包子了，买了回家");
		});
		Thread parkThread2 = new Thread(() -> {
			if (baozidian == null) {
				System.out.println("1.客户2没有包子，等它出锅");
				try {
					Thread.sleep(5000L);
				} catch (InterruptedException e) {
					e.printStackTrace();
			...
			System.out.println("3.客户2被通知有包子了，买了回家");
		});
		parkThread.start();
		parkThread2.start();
		Thread.sleep(1000L);
		baozidian = new Object();
		System.out.println("2.做好包子了，来买吧");
		LockSupport.unpark(parkThread);
		LockSupport.unpark(parkThread2);
	}
	public static void main(String[] args) throws Exception {
//		new Demo6().suspendResumeTestV1();
